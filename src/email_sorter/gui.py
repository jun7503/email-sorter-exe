...
from .mail_parser import parse_email_file  # ← if your file is mail_parser.py
...
class EmailSorterWindow(QWidget):
    ...
    def _choose_excel_path(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Choose Excel output", default_dir, "Excel (*.xlsx)"
        )
        if not path:
    ...
    def _open_excel(self):
        if self.excel and Path(self.excel.xlsx_path).exists():
            os.startfile(self.excel.xlsx_path)  # Windows-only
        else:
    ...
    def _handle_one(self, path: str):
        ...
        is_version = self.sim.is_version(clean_body, prev)  # currently unused

        dom = sender_domain(meta.get("From", ""))
        sub_idx = self.clusterer.assign(
            dom, canonical_subject(meta.get("Subject", "")), clean_body
        )
        topic_info = self.topic_map.ensure_domain_topic(self.excel.wb, dom, sub_idx)

        issue, milestone = extract_issue_milestone(
            meta.get("Subject", ""), clean_body
        )
